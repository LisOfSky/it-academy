...
function exTwo(strtNum, div, lim) {
    if (strtNum < lim) {
        ...
        console.log(`Задание 2. Получилось число ${strtNum}. Понадобилось ${num} итераций.`);
    }
}
...
//================================================= Exercise 5 =======================================================
// 5.1 (первоначальный вариант задания) Напишите функцию, которая вставит данные в массив в заданные несколько мест 
// в массиве. Дан массив [1, 2, 3, 4, 5]. Сделайте из него массив [1, 'a', 'b', 2, 3, 4, 'c', 5, 'e'].
...
function exFive(someArray, firstData, secondData, firstInsFrom, secondInsFrom) {    
    ...
    console.log(`Задание 5.1. Новый массив: ${someArray}`);
}
...
// 5.2 (новый вариант задания) Взять два массива и сделать таксировать элементы чередовались

const fourthArr = [1, 2, 3, 4, 5];
const data = [5, 4, 3, 2, 1, 0];

function exFiveNew(someArray, newData) { 
    //const newData = Array.from(arrayToInsert);
    let newArray = [];
    
    if (someArray.length === newData.length) {
        for (i = 0; i < someArray.length; i++) {
            newArray = newArray.concat(someArray[i]).concat(newData[i]);
        }
    } else if (someArray.length < newData.length) {
        for (i = 0; i < someArray.length; i++) {
            newArray = newArray.concat(someArray[i]).concat(newData[i]);
        }
        for (i = someArray.length; i < newData.length; i++) {
            newArray = newArray.concat(newData[i]);
        }
    } else {
        for (i = 0; i < newData.length; i++) {
            newArray = newArray.concat(someArray[i]).concat(newData[i]);
            }
        for (i = newData.length; i < someArray.length; i++) {
            newArray = newArray.concat(someArray[i]);
        }
    }
        
    console.log(`Задание 5.2. Новый массив: ${newArray}`);
}

exFiveNew(fourthArr, data);
